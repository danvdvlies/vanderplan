...
class FundingTests(TestCase):
    ...
    def test_fund_due_now_assigns_full_shortfall(self):
        ...
        added = services.fund_category(self.user, self.category, self.month)
        self.assertEqual(added, Decimal("220.00"))
        # Available now meets the target and nothing more is needed.
        self.assertEqual(
            services.category_available(self.user, self.category, self.month),
            Decimal("220.00"),
        )
        row = services.build_category_row(self.user, self.category, self.month)
    ...
    def test_fund_already_funded_does_nothing(self):
        self._goal(self.month)
        services.fund_category(self.user, self.category, self.month)
        added_again = services.fund_category(self.user, self.category, self.month)
    ...
    def test_fund_all_funds_every_short_category(self):
        ...
        self.assertEqual(
            services.build_category_row(self.user, self.category, self.month)["needed_this_month"],
            Decimal("0.00"),
        )
        self.assertEqual(
            services.build_category_row(self.user, groceries, self.month)["needed_this_month"],
            Decimal("0.00"),
        )
